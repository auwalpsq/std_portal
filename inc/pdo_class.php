...
Class DBConnection {
     ...
     */
    public function runCount( $sql ) {
        try {
          $stmt = $this->dbc->query($sql);
          $stmt->setFetchMode(PDO::FETCH_ASSOC);
        	$count = $stmt->rowCount();
          //$this->dbc->exec($sql) or print_r($this->dbc->errorInfo());
        } catch(PDOException $e) {
        	echo __LINE__.$e->getMessage();
        }
        return $count;
  ...
  /*
  Function get_p_query
  Runs select query with pdo parameters

  */
  public function get_p_Query($sql,$para,$p_value) {

    try {
      $stmt = $this->dbc->query($sql);

      $stmt->bindParam(':refid', $p_value, PDO::PARAM_STR,50);
$stmt->setFetchMode(PDO::FETCH_ASSOC);
      return $stmt;
    }catch(PDOException $e) {
      echo __LINE__.$e->getMessage();
